...
We [coded the algorithm](/pages/notes/danilevsky.qmd) considering the nature of arithmetic over $\FF$, which means that the characteristic polynomial of _large_ bit-matrices can be efficiently computed --- even for those with millions of entries that would choke more naive implementations.
