...
`bit` is a header-only {{< var cpp >}} library for numerical work in *bit-space* which mathematicians call [GF(2)] or $\FF$, the simplest [Galois field] with just two elements 0 & 1.
...
The library also has a [`bit::polynomial`] class to represent *bit-polynomials* over $\FF$.
...
Because arithmetic operations in $\FF$ are mod 2, addition/subtraction becomes the `XOR` operation, and multiplication/division becomes the `AND` operation.
...
The `bit::polynomial` class has methods to compute $x^N\bmod{P(x)}$ where $P(x)$ is a polynomial over $\FF$ and $N$ is a potentially huge integer.
