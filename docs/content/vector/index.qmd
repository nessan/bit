...
Macro | Description
----- | -----------
[`BIT_DEBUG`]               | This compile-time flag enables extra safety checks at the cost of performance.
[`NDEBUG`]                  | This compile-time flag disables all our safety checks --- even the cheap ones.
[`bit_debug_assertion`]     | These assertions are **only** checked if you set the `BIT_DEBUG` flag at compile time.
[`bit_assertion`]           | These assertions are checked **unless* you set the `NDEBUG` flag at compile time.
: {.bordered .striped .hover .responsive tbl-colwidths="[30,70]"}
