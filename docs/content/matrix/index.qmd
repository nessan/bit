...
```cpp
    using bit::matrix = bit::matrix<uint32_t>;
    bit::matrix mat = ...
```
...
In theory, there is no reason that one couldn't intermingle operations between, say, a `bit::matrix<uint32_t>` and a `bit::vector<uint64_t>`.
However, doing that efficiently makes the code much more complex.
...
[`matrix::polynomial_sum`]              | Evaluates a polynomial over GF(2) using a square bit-matrix as the argument.
